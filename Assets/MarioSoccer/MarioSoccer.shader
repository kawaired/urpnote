    ...
    SubShader
    {
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                float3 viewdir = _CameraPos - vertexws.xyz;
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                float4 metaltex = SAMPLE_TEXTURE2D(_MetalTex, sampler_MetalTex, i.uv);
                //return maintex;
                //return masktex.w;
                //return masktex.x;
                ...
                //return diffusefac;
                float4 diffusecolor = float4(lerp(0.6, 1.4, diffusefac) * maintex.xyz, 1);

                float ndoth = saturate(dot(cusnormal, i.halfdir));
                float specularfac = blinnphone(ndoth);
                
                //return specularfac;
                //float
                //return metaltex.w;
                //return masktex.z;
                return  diffusecolor*(1-metaltex.w)+ specularfac *maintex*2*masktex.x+metaltex.z*float4(1,1,1,1)*0.8;
            }
            ENDHLSL
        }
    }
